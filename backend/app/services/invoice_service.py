...
def generate_invoice_pdf(data: dict) -> BytesIO:
    ...
    address_p = Paragraph(
        "<b>Address:</b><br/>"
        "AJU Educational Solutions LLP<br/>"
        "Trivandrum, Kerala<br/>"
        "India<br/><br/>"
        "<b>Invoice No.</b>  INV-AJU-ITN-" + str(data.get('registration_id', '0000')) + "<br/>"
    ...
    if os.path.exists(LOGO3_PATH):
        try:
            logo_img = Image(LOGO3_PATH, width=1.4*inch, height=1.4*inch)
            logo_elements.append(logo_table := Table([[logo_img]], colWidths=[1.8*inch]))
            logo_table.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'RIGHT')]))
        except Exception:
            pass
    
    logo_elements.append(Paragraph("<b>AJU EDU SOLUTIONS</b>", logo_text_style))
    
    right_col = Table([[l] for l in logo_elements], colWidths=[1.8*inch])
    ...
    # 3. Customer Name & Address Table
    customer_header = [
        [Paragraph("Customer Name & Address", label_style)]
    ]
    ...
    customer_data = [
        ["Name", Paragraph(data.get('name', 'N/A'), styles['Normal'])],
        ["Email Address", Paragraph(data.get('email', 'N/A'), styles['Normal'])]
    ]
    ...
    # 5. Grand Total Row
    total_data = [
        ["", "GRAND TOTAL", amount_val],
        ["", "Total Charge", amount_val]
    ]
